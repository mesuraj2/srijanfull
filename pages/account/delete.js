...
const DeletePage = () => {
  ...
  const handleDelete = async () => {
    const response = await axios.post(
      'https://script.google.com/macros/s/AKfycbw6GMdKiF_s1rtFroX2EuYMWzMTviulvJa0zXLqe-q-vVn8HCNUrRL9MhPu5cCDStLW/exec',
  ...
  return (
    <div
      className="w-screen bg-[#B9E9FC]"
      style={{
        minHeight: 'calc( 90svh - 100px)',
        display: 'flex',
        alignContent: 'center',
      }}
    >
      <div className="flex flex-col justify-center items-center gap-10 pt-[2rem] pb-[5rem] mx-auto px-4 md:px-0">
        ...
        <div className="form-control w-[90%] mx-auto 5xl:w-[28rem]">
          <p className="text-black/70">
            You will recive an otp for this account
          </p>
          <input
            value={email}
            onChange={(e) => {
          ...
          <div
            ...
            className="secondary_font text-black/70"
          >
            <input
              type="checkbox"
              checked={sure}
              onChange={(e) => {
                console.log(e.target.checked), setsure(e.target.checked);
              }}
            />
            <span
          ...
          >
            <input
              ...
              checked={unsub}
              onClick={(e) => {
                console.log(e.target.checked), setunsub(e.target.checked);
              }}
            />
            <span
              style={{ cursor: 'default' }}
